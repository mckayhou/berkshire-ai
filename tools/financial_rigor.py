...
Used by investment analysis workflows (standalone prompts, local loops, or any agent) at critical validation checkpoints.
...
# Safe arithmetic AST evaluator (no eval) ------------------------------------
...
def _safe_eval_node(node):
    """Recursively evaluate an arithmetic AST, rejecting anything unexpected."""
...
def safe_arith_eval(expr: str):
    """Evaluate a pure arithmetic expression safely (no builtins, no names)."""
...
def exact_calc(expr: str):
    ...
    Uses a restricted AST evaluator instead of eval() for safety.
    """
    print("=" * 60)
