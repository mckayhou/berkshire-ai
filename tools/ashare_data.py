...
为投资分析工作流（独立提示模板、本地循环或任意 Agent）提供 A 股实时行情、财务数据等数据。
...
def _fmt_num(value, ndigits: int = 2) -> str:
    """安全数值格式化：兼容 API 返回的字符串/数字，无法解析时原样返回。"""
    if value is None or value == "-" or value == "":
        return "-"
    try:
...
def cmd_valuation(code: str):
    ...
    # 由市值反推总股本（仅作参考）。
    # 注意：这里的 shares = 市值 / 股价 是反推值，再乘回股价必然等于市值，
    # 因此【不能】用它来"验算"市值是否正确（那是循环论证）。
    # 真正的市值校验需用独立来源的总股本，请用：
