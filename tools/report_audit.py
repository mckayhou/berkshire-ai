...
工作流程（三步）：
  ...
  Step 2 — Agent/研究者对抽检清单中的每个数据点，从可靠信源（macrotrends/
...
工作流程：
  ...
  Step 2 — Agent/研究者对清单中每个数据点，从可靠信源取数，
  ...
  固定随机种子（复现同一批样本）：
    ...
    if args.command == 'extract':
        ...
        try:
            ...
            print(f'❌ results JSON解析失败: {e}', file=sys.stderr)
            sys.exit(1)

        verdict = render_verdict(results, args.report)
