...
description: |
  股价异动快速新闻归因框架（10-15 分钟情报响应）。
  四维度侦察（公司事件、监管、行业、情绪）+ 归因判断 + 行动建议。
  适合持仓监控或突发异动响应。
version: 10.2
---
...
**激活条件**：用户报告股价大涨/大跌（±5%+）、财报后异动、或问“最近发生了什么导致这个波动”时激活。

对 $ARGUMENTS 进行最近新闻侦察与异动归因。**这不是深度投研，是情报快速响应**——目标是 10 分钟内回答："这家公司最近发生了什么？股价异动的真因是什么？要不要重审投资论文？"

**本技能完整保留上游的多 Agent 并行侦察团队方式**（team-lead + 4 个并行 specialist scout），只是把 Claude Code 语法换成 OpenClaw/QwenPaw 支持的子 agent 并行 + 通信方式。

OpenClaw：用 sessions_spawn / ACP spawn 4 个并行子 agent 做侦察，通过 ACP 消息发报告回 lead。
QwenPaw：用 loop 的并行能力启动 4 个角色并发跑。
...
### 第三步：四角色侦察模板（Standalone / 并行使用）

**推荐用法**：
- 单会话：让一个强大模型按下面四个侦察维度依次执行。
- 并行会话：将四个角色提示分别复制到四个对话窗口并行执行，再把结果汇总给主会话合成。

下面是四个维度的**任务描述 + 独立提示模板**。
...
- **description**:
  ...
  6. **技术面 / 资金面信号**：是否触及关键支撑/阻力。A 股用
     `python3 tools/capital_flow.py score {代码} --json`
     看大宗、融资融券、龙虎榜是否异常（模块缺失则写未取到，禁止编）。
  7. 关键判断：**是基本面驱动还是情绪/资金面驱动？**

### 第五步：在 OpenClaw / QwenPaw 中启动 4 个并行侦察子 Agent

**team-lead（你）** 使用平台的多 agent 能力真正并行启动 4 个 specialist：

- OpenClaw: sessions_spawn 或 ACP spawn 4 个子 agent（company-event-scout, regulatory-watcher 等）。
- QwenPaw: 启动 4 个并行角色实例。

把对应角色提示赋给它们。

要求它们完成后通过消息把报告发回 lead。

lead 收集 4 份，做归因合成。

（如果平台不支持轻松并行子 agent，则顺序或手动并行会话执行角色提示。）
...
**侦察方法**：
- 优先 **hybrid Tavily**（`SEARCH_MODE=hybrid python3 src/tavily_search.py news ...` 或等价）做时效性查询；失败/需长正文再 AnySearch Skill（`anysearch-web.md`）或 WebSearch（关键词加日期或「最近」「latest」「2026」）
...
5. 严格区分"事实"与"推测"，所有判断必须附来源

**完成后**：
请输出完整的 Markdown 格式侦察报告（核心发现 + 时间线表格 + 本维度归因结论 + 信息缺口）。在并行环境中，把报告发回主会话用于最终归因。
```

### 第六步：综合归因（主会话统筹）

收集齐 4 份侦察报告（或单会话内部完成全部维度）后，直接进入综合归因：
...
| 追加到 Pending Queue（`config/state.md` §2） | | 见下方队列模板 |
| 触发深度财报研读（`/earnings-review`） | | |
| 触发管理层重审（`/management-deep-dive`） | | |
| 调仓动作（加仓/减仓/不动） | | 仅做提示，最终决策权在用户 |
| 仅观察 | | |

**若建议入队**，在报告末尾附上可粘贴条目（与 `thesis_queue` 格式一致）：

```markdown
- [ ] **{代码}**: {异动摘要} (Triggered by news-pulse, {日期})
```

价值事件或「真因不明」且持仓相关时，**默认建议入队**；纯情绪/技术波动且论文未变时可不入队。

运行 `python3 tools/thesis_queue.py --json` 查看当前队列优先级，避免重复条目。
