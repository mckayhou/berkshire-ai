...
description: |
  未上市公司多Agent并行深度研究框架（6 专业子 Agent：商业模式/财务侦探/竞争格局/风险治理/技术IP/替代数据信号）。
  在信息天然稀缺的条件下还原公司的真实价值。
  完整支持多 Agent 并行团队运行（OpenClaw/QwenPaw 风格），已移除 Claude Code 特定 Team/Task 语法。
version: 10.2
---

# 未上市公司研究：多Agent并行深度研究框架

**激活条件**：当用户要求对一家**未上市公司**（如蚂蚁集团、小红书、SpaceX、Stripe、字节跳动等）进行深度投研、估值推演、或“私募/一级市场研究”时，激活本技能。
...
## 多 Agent 并行运行说明（OpenClaw / QwenPaw）

本技能完整保留上游“6 专业子 Agent 并行研究”的方式：一个 **team-lead** + 6 个专业子 Agent 真正并发工作，各自独立研究后把完整报告发回 lead 做交叉验证与汇总。

我之前清掉了 Claude Code 的 `TeamCreate`/`TaskCreate`/`Agent`/`TeamDelete` 指令，但**多 Agent 并行运行的核心流程没有丢**，只是改用 OpenClaw / QwenPaw 这类产品支持的方式实现：

- **OpenClaw**：用 `sessions_spawn`（或 `/acp spawn`，带 thread/subagent）为 6 个角色分别创建独立子 Agent / 子会话，并行执行；子 Agent 完成后通过 ACP 消息 / `sessions_send` 把完整报告发回 team-lead；lead 收齐 6 份后做交叉验证 + 汇总，完成后发送 close / shutdown。
- **QwenPaw / 同类**：在你的 loop_engine / agent harness 中启动 6 个并发角色实例（或使用内部多 agent 调度），让它们并发跑角色提示 + 工具调用，报告通过你的消息通道传回主 lead 循环。
- **单 agent 降级模式**（平台不支持轻松 spawn 并行子 agent 时）：把 6 个角色提示顺序或分段交给一个强模型完成。工具调用与最终报告结构不变。

**不要把所有工作压在一个 Agent 上** —— 并行是这个技能的核心价值（六个不同专业视角同时深度工作、互相印证）。
...
### 第二步：用平台多 Agent 能力创建并行研究团队

1. 你（当前会话）作为 **team-lead**（统筹者）。
2. 使用平台的并行/子 Agent 机制启动 6 个专业子 Agent：
   - OpenClaw：用 `sessions_spawn` 或 `/acp spawn` 为每个角色创建独立会话/子 Agent，并行运行。
   - QwenPaw：在 loop_engine 中启动 6 个并发角色实例，或通过 harness 的并行调度启动 6 个 specialist agents。
3. 给每个子 Agent 分配下面的角色提示 + 具体任务描述（第三步的 6 个任务块）。
4. 要求子 Agent 完成独立研究后，通过消息（ACP / `sessions_send` / 共享上下文）把完整报告发回 team-lead。

### 第三步：为 6 个子 Agent 定义任务

下面是 6 个任务的定义（subject / activeForm / description）。把每个任务的 description 直接赋给对应的子 Agent。
...
通过平台多 Agent 能力（OpenClaw `sessions_spawn` / QwenPaw harness 并发）**同时启动 6 个子 Agent**——这是本技能的核心价值，不要退化成一个 agent 串行模拟（除非平台不支持）。
...
4. 使用 WebFetch / browse 获取关键文章的全文（不要只看搜索摘要）
...
**完成后**：把完整报告通过平台通信方式发回 team-lead（OpenClaw 用 ACP 消息 / sessions_send；QwenPaw 用 harness 消息机制），等待 team-lead 结束指令后再停止。
```
...
**这是增强版框架最关键的步骤**。在汇总前，team-lead 需要：

1. **数据冲突仲裁**：
   - 提取各Agent报告中的关键数据
   - 识别不同Agent引用的相同数据是否一致
   - 对冲突数据进行仲裁：列出所有来源，说明采信哪个、为什么
   - 涉及数字的，用 `python3 tools/financial_rigor.py cross-validate` 复核
...
全部报告收到并汇总后，向 6 个子 Agent 发送结束指令（OpenClaw 用 ACP shutdown / sessions close；QwenPaw 用 harness 的对应机制）。

---

## 重要注意事项

1. **6个Agent尽量并行启动**——在支持的平台同一批次并发 spawn 6 个子 Agent；不支持时降级为单 agent 顺序模拟，但工具调用与报告结构不变
2. **数据置信度标注**——未上市公司数据来源参差不齐，每个关键数据必须标注来源和置信度
3. **推算要透明**——所有推算过程要展示计算逻辑，不能凭空给数字
4. **交叉验证**——关键数据至少2个来源交叉验证，来源冲突时都列出，数字用 `tools/financial_rigor.py` 复核
