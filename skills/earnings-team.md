# 财报精读团队：四大师并行解读 + 公众号发布框架

对 $ARGUMENTS 进行团队化财报精读分析。四位大师视角并行解读财报，编辑润色成文，读者评审把关质量，最终产出可直接发布的公众号文章。

**本技能完整保留上游的多 Agent 团队方式**（研究阶段 4 大师并行子 agent + 编辑 + 读者评审并行），只是把 Claude Code 语法换成 OpenClaw/QwenPaw 支持的子 agent / 并行机制。

阶段一研究时用并行子 agent 跑 4 个视角，阶段三用并行子 agent 做编辑和评审。
...
**推荐**：让主 Agent 或并行角色直接使用 WebSearch / browse 工具获取以下原始材料（或手动提供给模型）：
...
### 第三步：使用平台多 Agent 能力启动四个并行研究子 Agent

**在 OpenClaw / QwenPaw 中**：

- 作为 team-lead，使用 `sessions_spawn` / ACP spawn / loop 并行机制启动 4 个研究子 agent（生意本质、财务质量、竞争格局、风险信号）。
- 把对应角色提示赋给每个子 agent，让它们**真正并行**执行（尤其是财报季多个公司时更有价值）。
- 子 agent 完成后把完整解读报告通过平台消息发回 lead。
- lead 收集后进入阶段二合成。

如果你的平台暂时不支持方便的子 agent spawn，可以让主 agent 顺序跑 4 个角色或手动开多个会话。
...
在单会话模式下，主 Agent 自己跟踪并展示进度。
在并行模式下，当四个角色报告返回后，在主会话中展示汇总进度，然后进入阶段二合成。

每收到一份角色报告，提取 3-5 条核心发现并向用户展示。
...
研究报告完成后，执行两个后续结构化角色（可以是同一个会话内继续、或新会话）：

**执行方式**：单会话顺序完成，或启动两个额外会话（编辑 + 读者评审），把研究底稿作为输入。
