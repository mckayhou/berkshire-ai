...
description: |
  深度公司系列：3–8 篇长文拆解一家公司（按复杂度定篇数）。事实核查严格。
  含篇内结构骨架（上游 xbtlin 增量）。
version: 10.29.3
---

# 看懂XX公司（深度公司系列）：3-8 篇长文拆一家公司

为 $ARGUMENTS 撰写一个《看懂XX》深度长文系列（**3–8 篇，按公司复杂度定**），发布在公众号/视频号等公开渠道。**核心 IP 不是"会写"，而是"会改"——99% 的财经文章在违反本 skill 的事实核查标准**。
...
**篇数不固定为 8。** 8 篇是腾讯这种「多业务 + 万亿投资组合 + 20 年管理层故事」才撑得起的容量。写之前先问：这家公司有几个能用「一个尖锐问题」独立成文的主轴？有几个写几篇。
...
判断信号：如果某篇写到一半在「凑内容」（同一事实换说法重复），篇数定多了——合并。反之某篇超过 12,000 字还塞不下，拆开。
...
**篇目必须适配公司**：模板是腾讯长出来的，不是套子。某篇没有独立内容（如没有投资组合写不了 04），就替换成该公司特有主轴，或按上面的合并方式折进更少篇数。

---

## 二·五、篇内结构模板（从《看懂腾讯》提炼 · 上游增量）
...
- **结尾两行斜体**：系列归属 + 「基于公开信息与价值投资框架，不构成投资建议」

各篇专属骨架：

| 篇 | 骨架 |
|---|------|
| 01 开篇 | 股价被市场反复「重新定性」的历史 → 三个常见错觉（各用财报数据破）→ 阅读地图 → 估值钩子（只留悬念） |
| 02 护城河 | 挑战者失败名单（有名有姓有年份）→ 迁移成本本质 → 生态飞轮 → 5/10 年护城河 → 「有 1000 亿能否复制」实验 |
| 03 利润引擎 | 反常识数字 → 定价权 → 增长空间（含反面论据）→ 组织能力 → 市场为何低估 → 时代变量 → 5 年后图景 |
| 04 隐藏资产 | TOP 持仓表（**只统计未并表**，防双算）→ 会计处理差异 → 不会卖 vs 可能减持 → 市场折扣原因 → 估算方法 |
| 05 AI/时代变量 | 市场担心的数字 → 分业务赢家/输家 → 关键反问 → 被绕过场景 → 情景与监控信号 |
| 06 财务 | 反常曲线 → 利润表/口径 → EPS 与回购 → BS/FCF → 1 美元测试 → ROE → 估值比较 → Checklist → 三句话压缩 |
| 07 管理层 | 核心团队 → 具体决策贡献 → 利益对齐 → **诚信检验：挑一次真实危机** → 承诺兑现率 → 资本配置 → 接班人 → 买人三问 |
| 08 决策 | SOTP（说清买的是什么）→ 历史分位 + **后视镜不是导航仪** → 三情景（标明预测）→ 红线清单 → 赔率思维 |
...
4. 与用户确认**篇数与**各篇核心论点（避免写完才发现方向不对）
...
- 用户提修订意见后修改
- 修订完才 git push
- 数字验算用 `tools/financial_rigor.py`；终稿抽检用 `tools/report_audit.py`
...
- 推送前必须用 grep 扫描 `linxuan` / `/Users/` / 用户公司花名 等隐私字段（参见任何本地记忆/feedback 文件）
- 公开署名按用户多层身份策略，不混用
