...
#### 7½. Portfolio Manager 汇总 — 行动卡（必须）

team-lead 在合成四份子报告后，除上述章节外，**必须**输出一页「行动卡」（模板：`docs/action-card.md`）：
- 综合立场 + 操作建议 + **建议仓位区间**（与 `stock_screener` / `portfolio_scan` 分级可对照，但不机械等同）
- 目标价区间、置信度（区分 AI 分析 vs 投资确定性）
- 看多/看空 ≤3 条、风险 Top 3、催化剂与论点失效条件

这是借鉴 ai-hedge-fund Portfolio Manager 的**可执行结论层**，但输出仍是研报向建议，非自动交易指令。
