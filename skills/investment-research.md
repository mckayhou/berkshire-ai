...
> - A股：东方财富（主）+ 巨潮资讯（副）
>
> **联网检索（V10.29.2）**：优先 **AnySearch Skill**，再 hybrid 回退 Tavily。禁止用 LLM 内部知识填财务数字。
>
> ```bash
> # Agent 首选
> python3 skills/anysearch/scripts/anysearch_cli.py search "{公司} {ticker} 市值 PE" --max_results 5
...
> ```
>
> 详见 `skills/anysearch-web.md`、`docs/USER_GUIDE.md` §5.4。Key：`ANYSEARCH_API_KEY` / `TAVILY_API_KEYS`（仅 `.env`）。
