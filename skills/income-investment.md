---
name: berkshire-income-investment
description: |
  股息/可分配收益专项：耐久 vs 机会性收益、覆盖与杠杆、税制与组合适配；不把高显示收益率当机会证据。适合 OpenClaw / QwenPaw。
version: 10.29.3
---
...
## OpenClaw / QwenPaw 适配（本 fork）

- 本 skill 自 [xbtlin/ai-berkshire](https://github.com/xbtlin/ai-berkshire) 移植；运行时按 **OpenClaw / QwenPaw** 使用，不依赖 Claude Code 专有 Team/Task API。
- 数据规范：必须遵循 `skills/financial-data.md`（含台股 `tools/twstock_data.py`）。
- 数字验算：`python3 tools/financial_rigor.py ...`
- 报告准出：`python3 tools/report_audit.py extract|verdict ...`
- **正式研究收尾**（若形成可执行观点）：`python3 tools/log_decision.py append ...`（建议 `--strict`；action↔mean_stance 带宽见 `log_decision.py bands`）。
- 安装：`./update-platforms.sh` 同步到 OpenClaw workspace。
