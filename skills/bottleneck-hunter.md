...
description: |
  供应链瓶颈猎手：从物理供应链咽喉位置挖掘第二三层 alpha 机会。
  超级趋势确认 + 多层瓶颈扫描 + 公司映射 + 投资论点。
version: 10.2
---
...
**台股标的取数**（供应链扫描常命中 4 位代码，如 2330、3661）：行情/PER/市值/月营收统一用 `python3 tools/twstock_data.py`（FinMind，见 `skills/financial-data.md` 台股节）。台股每月约 10 日前强制披露上月营收，**月营收同比**是验证瓶颈「量价齐升」最快的公开信号。
