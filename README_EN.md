...
**Current version**: **V10.19** (R/D dual-loop `src/research_loop.py`: `HypothesisProposer` + `run_rd_cycle` proposes falsifiable hypotheses then runs validation-gated prompt evolution; `proposer=None` degrades to pure D; experience retrieval wired through D-phase rewriting; optional `hypothesis_id` on `decision_log`; cumulative with V10.18 RD-Agent/Qlib borrowings + V10.17 deployment tier + production hardening A→D). Full history in [VERSION_HISTORY.md](VERSION_HISTORY.md).
