...
**Current version**: **V10.18** (three minimal increments borrowed from RD-Agent / Qlib: a stdlib-only performance metrics library `tools/perf_metrics.py` (Qlib `risk_analysis` conventions: annualized return/volatility/IR/Sharpe/max-drawdown/sum-based cumulative/excess CAR/with-cost) + a RAG-lite experience store `src/experience_store.py` (JSONL persistence of win/loss signals + deterministic keyword retrieval + few-shot injection into `build_rewrite_messages`) + an explicit `Hypothesis` first-class object `src/hypothesis.py`; cumulative with V10.17 containerization/access-control/metrics/∇_LLM real gradient + tier A CI/packaging + tier B validation-gated rewriting/real-market data/eval harness + tier C observability/service/injection-defense). Production hardening tiers A→B→C→D all shipped. Full history in [VERSION_HISTORY.md](VERSION_HISTORY.md).
...
**Borrowed from RD-Agent / Qlib (V10.18)**: three minimal, dependency-free increments grounded in the read-only evaluations (`docs/qlib_evaluation.md`, `docs/rdagent_reference.md`). (1) `tools/perf_metrics.py` — a pure-stdlib performance library adopting Qlib's `risk_analysis` conventions (annualized return/volatility, information ratio/Sharpe, max drawdown, **sum-based** cumulative return to avoid exponential distortion, win rate, benchmark-relative excess/α, with/without cost), bridging `decision_log` snapshots + an injectable/mockable `PriceProvider` into a NAV/excess curve — pure functions, offline-testable, `render_markdown`/`to_json`. (2) `src/experience_store.py` — a RAG-lite knowledge base turning the previously-discarded `realized_feedback` win/loss signals into retrievable `Experience` records (JSONL, `decision_log`-style), with a zero-dependency deterministic `KeywordExperienceRetriever` and injection as few-shot into `build_rewrite_messages(..., examples=None)` (byte-identical output when `examples=None`, sanitized, graceful degradation). (3) `src/hypothesis.py` — an explicit falsifiable `Hypothesis` first-class object + minimal store, paving the way for a future R/D dual loop (object + storage only this round, not wired into the main chain). Explicitly **not** adopted: CoSTEER code-gen + Docker sandbox, multi-trace scheduling / web viewer, qlib factor/ML/binary-data stack / qrun / RL / portfolio-optimization direct dependencies.
