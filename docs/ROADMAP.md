...
### 测试覆盖 + 工程门禁 — 🟢 大幅完善（V10.17 收紧，V10.18–10.21 扩充）
- **407 pytest**（引擎、scenario、reflect、run_recorder、evolution_cli、research_loop、prompt_optimizer（含 few-shot 注入）、config、prompt_validation、eval_harness（含 golden 回归）、observability、sanitize、service（含鉴权/限流/指标）、access_control、metrics_export、llm_gradient、**perf_metrics**、**experience_store**、**hypothesis**、financial_rigor、report_audit、网络层、portfolio_*、thesis_queue、收益反馈闭环、data_sources、notify、**磁盘价格缓存**）
...
- ✅ V10.21 Scenario 抽象：`src/scenario.py` + `BerkshireGraph(scenario=DEFAULT_SCENARIO)`（默认行为不变）
- ✅ V10.21 进化 CLI：`status` / `reflect <ticker>` / `optimize <ticker>`（`src/evolution_cli.py` + `src/reflect.py`）
- ✅ V10.21 Run Recorder：`src/run_recorder.py`（JSONL，`BERKSHIRE_RUN_LOG`）
- 待补：Cron 自动进化、QwenPaw 侧轨迹自动记录（见 `config/skill.md`）
...
- ✅ 显式假设对象 `src/hypothesis.py`（借 RD-Agent 一等公民 Hypothesis）：`Hypothesis`（可证伪命题）+ `HypothesisStore` + `group_experiences_by_hypothesis`（经验按假设聚合预留接口）。（对应 rdagent P0-B / 切口二）
- ✅ V10.21 Scenario 抽象 `src/scenario.py`（P1-D）
- ✅ V10.21 轻量 Run Recorder `src/run_recorder.py`（Qlib B1）
- ✅ V10.21 `NetworkPriceProvider` 磁盘缓存（Qlib B3，`BERKSHIRE_PRICE_CACHE_DIR`）
...
- 待补（按需）：HTML 报告、多股对决矩阵、Cron 自动进化
